 ...
 *
 * Matching is by canonical URL identity (`normalizeSourceUrl`). A playlist
...
import { normalizeSourceUrl, pickKeeper } from './dedup'
...
function unitIdentity(unit: IngestUnit): string | undefined {
  if (unit.kind === 'youtube') return normalizeSourceUrl(unit.url)
...
export function reconcileUnits(
  ...
  for (const source of sources) {
    if (source.url === undefined || source.url.trim() === '') continue
    const identity = normalizeSourceUrl(source.url)
